...
{ ?>
    <label class="woocommerce-form__label woocommerce-form__label-for-checkbox checkbox chek_confirmed">
        <input type="checkbox" class="woocommerce-form__input woocommerce-form__input-checkbox input-checkbox"
               ...
               id="terms"/> <span>Я прочитал и принимаю условия <a href="//printblog.ru/politika"
                                                                   class="woocommerce-terms-and-conditions-link">политики конфидециальности</a></span>
    </label>
...
function my_sold_out_loop()
{
    ...
    if ($product->is_in_stock() && $product->get_stock_quantity() == 0) {
        ...
        echo '<div class="cat-out-of-stock" >Доступно под заказ</div>';
    }
}
...
/* Выводим код товара в каталоге 
 function rupom_custom_price_sku( $price ) { 
  ...
  if (is_shop() || is_product_category() || is_product_tag()){
      return $price . '<br /><span class="price-sku" style="color:red">Код товара ' . $sku . '</span>';
  }
...
function check_woocommerce_status_after_change($order_id, $old_status, $new_status)
{
    ...
    $setCancelledStatusEventMetaKey = 'set_cancelled_status_event_timestamp';

    if ($new_status === 'on-hold') {
        $setCancelledStatusEventTimestamp = time() + 172800; //172800 секунд = 2 суток
        wp_schedule_single_event($setCancelledStatusEventTimestamp, SET_ORDER_STATUS_TO_CANCELLED_HOOK_NAME, $hookArgs);
        update_post_meta($order_id, $setCancelledStatusEventMetaKey, $setCancelledStatusEventTimestamp);
    } else {
        $timestamp = get_post_meta($order_id, $setCancelledStatusEventMetaKey, true);
        wp_unschedule_event($timestamp, SET_ORDER_STATUS_TO_CANCELLED_HOOK_NAME, $hookArgs);
        delete_post_meta($order_id, $setCancelledStatusEventMetaKey);
    }
}
